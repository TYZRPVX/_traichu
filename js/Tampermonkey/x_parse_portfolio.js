...
(function() {
    ...
    function parsePortfolioText(text) {
        ...
        // $347.82              <- PRICE (line 2)
        // 6                    <- QUANTITY (line 3) *** WE WANT THIS ***
        // +$2.64               <- CHANGE_VALUE (line 4)
        // 0.13%                <- PERCENTAGE (line 5)
        // $2,086.92            <- TOTAL_VALUE (line 6)
        ...
        while (i + 6 < lines.length) {
            ...
            if (isValidSymbol && isValidPrice && isValidQuantity) {
                // Clean quantity - remove commas, keep only numbers and dots
    ...
    function convertToCSV(data) {
        ...
        const header = 'SYMBOL,QUANTITY';
        const rows = data.map(item => `${item.symbol},${item.quantity}`);
